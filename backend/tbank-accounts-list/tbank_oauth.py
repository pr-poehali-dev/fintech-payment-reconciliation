...
def fetch_company_info(access_token: str, is_sandbox: bool = True) -> Optional[Dict[str, Any]]:
    ...
    Наличие успешного ответа здесь и есть признак того, что владелец подтвердил
    доступ через Т-Бизнес - на основании этого решаем, показывать ли ручной ввод
    счёта/токена или сразу предложить выбрать счёт из автоматически полученного списка.
    '''
...
def fetch_statement(access_token: str, account_number: str, date_from: str,
    ...
    В песочнице банк принимает фиксированный Bearer TBankSandboxToken и всегда
    отдаёт один и тот же демонстрационный набор операций - этого достаточно,
    чтобы проверить весь пайплайн разбора, классификации и сохранения данных.
    '''
